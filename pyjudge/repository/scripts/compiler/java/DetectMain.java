...
public class DetectMain {
  public static void main(String[] args) {
    // Check arguments
    if (args.length < 2) {
      System.err.println("Usage: java DetectMain <dir> <class1> [ <class2> ... ]");
      System.exit(1);
      return;
    }
    ...
    for (int i = 1; i < args.length; i++) {
      ...
      if (Modifier.isStatic(mainMethod.getModifiers())
        ...
        if (result == null) {
          System.err.printf("Info: using 'main' from '%s'%n", arg);
          result = arg;
        } else {
          System.err.printf("Warning: found another 'main' in '%s'%n", arg);
        }
      }
    }

...
class AppConfig:
    ...
    def load(cls, path: pathlib.Path) -> "AppConfig":
        ...
        return cls(
            ...
            trust_proxy=int(server.get("trust_proxy", 0)),
...
def create_app(config: AppConfig | None = None) -> Flask:
    # Fallback to loading from an environment variable if no config is passed
    if config is None:
        config_path = pathlib.Path(os.environ.get("APP_CONFIG_PATH", "config.yaml"))
        if not config_path.is_file():
            raise ValueError(
                f"No configuration found at {config_path}, create one or set APP_CONFIG_PATH environment variable"
            )
        config = AppConfig.load(pathlib.Path(config_path))
    ...
    if config.trust_proxy:
        ...
        app.wsgi_app = ProxyFix(
            app.wsgi_app,
            x_for=config.trust_proxy,
            x_proto=config.trust_proxy,
            x_host=config.trust_proxy,
            x_port=config.trust_proxy,
            x_prefix=config.trust_proxy,
        )
...
def start() -> None:
    parser = argparse.ArgumentParser(description="DOMjudge self-service")
    parser.add_argument(
        "-c",
        "--config",
        type=pathlib.Path,
